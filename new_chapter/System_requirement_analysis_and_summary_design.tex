...
整个地理轨迹数据服务的核心功能就是对相似路径的检索，因此轨迹数据服务的功能需求有两个。一个是对批量轨迹数据导入建立索引的功能，另一个是Trajectory KNN功能，即根据目标轨迹的ID，执行检索算法，找出与目标轨迹最相近的K 条轨迹。
...
瓦片数据服务的部分功能操作简单明确，无需使用用例描述。本文只对操作比较复杂的“b-box更新地图瓦片数据”和“b-box瓦片数据渲染”进行了具体描述。
