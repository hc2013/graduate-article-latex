...
R4   & 单个地图瓦片数据查询                  & \begin{tabular}[c]{@{}l@{}}服务调用方能够通过瓦片服务，参数为地图ID和栅格坐标zxy，获取\\ 指定的瓦片数据.\end{tabular}     \\ \hline
\end{tabular}}
...
\textbf{ID}&\tabincell{l}{UC3}\\
\hline
\textbf{名称}&\tabincell{l}{单个地图瓦片数据查询}\\
\hline
\textbf{参与者}&\tabincell{l}{普通用户}\\
\hline
...
单个地图瓦片的查询功能与地图瓦片全量查询功能的调用情况是相同的，其最可能出现异常的操作就在于zoom值与x,y值的设定上。如果zoom超出服务范围或者x,y超出zoom范围，都不能正常获得瓦片数据。
