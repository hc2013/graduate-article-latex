...
\cite{DBLP:journals/classification/ProencaN17}这是距离范围剪枝的基本依据。显而易见的是，这个容忍距离越小，成功剪枝的概率越高，检索性能越好。但是如果容忍距离太小，又可能过度剪枝造成检索不到结果。所以容忍距离的初始值和收敛速度是直接影响检索性能的关键。\cite{DBLP:conf/hucc/ZengLJLCM14}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=6in,height=6.8in]{new_FIGs/chapter4/hausdoff-dis-class.pdf}
  \caption{轨迹距离计算设计类图}\label{trajectory-distance-compute-class}
\end{figure}
本文主要针对Geometry类中LineString（线条）和Polygon（几何体）两种类型进行设计，因为轨迹数据基本上都属于这两种类型。对于前文所提到的点集合全集与目标的距离计算无法实现的问题，本文采用了致密度计算这个方式进行这种，其核心思想是将一根线条按照一定的比例切分成数段，将每一段的两端点坐标纳入距离计算，从而最大限度保留了整根线条的距离属性。
\section{地图瓦片服务详细设计}
\subsection{概述}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=6in,height=4.0in]{new_FIGs/chapter4/consistancy-code.pdf}
