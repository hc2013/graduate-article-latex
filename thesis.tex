...
针对以上问题，本文设计和实现了一个高性能的地理轨迹相似性分析服务。该服务包括两个主要的数据子服务，轨迹数据子服务和地图瓦片数据子服务，具体的实现方法是基于优先点树结构扩展实现全文检索库Lucene的VpTree索引，
    ...
    地理轨迹相似服务正是从以上这两个子服务中分别获取轨迹数据和地图瓦片数据，然后通过前端库的整合，实现在浏览器端的可视化，从而使用户方便快捷地达成相似轨迹检索的目标。
...
In response to the above problems, this thesis designs and implements a high-performance geographic trajectory similar service. The service includes two main data
...
The geographic trajectory similar service obtains the trajectory data and the map tile data from the above two sub-services respectively, and then realizes the
...
\begin{enumerate}[label=\arabic*., labelindent=0em, leftmargin=*]
	\item 刘海涛，\textbf{韩淳}，``基于优先点树的地理轨迹数据服务的设计和实现''，申请号：20xx1018xywz.a，已受理。
\end{enumerate}
